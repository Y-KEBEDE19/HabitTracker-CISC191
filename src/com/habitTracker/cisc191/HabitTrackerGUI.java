...
public class HabitTrackerGUI extends JFrame
{
	...
	private ImageIcon logo = new ImageIcon(getClass().getResource("/Images/habity.png")); // Logo Image
	...
	private ImageIcon addHabitImg = new ImageIcon(getClass().getResource("/Images/addButton2.png")); // habitButton Image
	private JPanel detailPanel = new JPanel(); // When clicking on a habit it brings up a detail panel 
	private Habit currentlyShowingHabit; // Current Habit whose details are being shown
	...
	private ImageIcon saveIcon = new ImageIcon(getClass().getResource("/Images/saveButton.png")); // Save Button Image
	private JButton loadHabits; // GUI Has- A loadHabits Button
	private ImageIcon loadIcon = new ImageIcon(getClass().getResource("/Images/loadButton.png")); // load Button Image
...
//		instructions.setPreferredSize(new Dimension(50,50));
		...
		saveHabits = new JButton(saveIcon); // Insert Image of Save Icon into Save Habits
		loadHabits = new JButton(loadIcon); // Insert Image of Load Icon into Load Habits
	...
	private void addHabitCard(Habit habit)
	{
	  ...
	  JLabel title = new JLabel(habit.getName()); // Set the Name of the Habit to User Input
	 ...
	 check.addItemListener(evt -> // if CheckBox is clicked mar & Unmark habit completion 
	  	{
	  ...
	  card.addMouseListener(new MouseAdapter() // If card is clicked show details 
	  {
  	   	  @Override
	...
	private void showHabitAnalytics(Habit habit) 
	{
	    ...
	    detailPanel.add(new JLabel("Days Tracked:   " + habit.getDaysTracked()));
	    detailPanel.add(new JLabel("Longest Streak: " + habit.getLongestStreak()));
	    detailPanel.add(new JLabel("Current Streak: " + habit.getCurrentStreak()));
	...
	{				
		JPanel form = new JPanel(new GridLayout(2,2,5,5)); // Create A Form to get Title of FileName
		JTextField nameFld = new JTextField(); // Create a Name Field
	...
	private void loadHabitForm() 
	{
	    ...
	    recentButton.addActionListener(e -> fileNameField.setText(lastFileName));

	    Component [] component = 
	    	{
	    form.add(new JLabel("File Name:")),
	    form.add(fileNameField),
	    form.add(recentButton),
	...
	private void loadHabit(String fileName)
	{
		...
		try
		{
			fileManager = new FileManager(trackerModel,lastFileName);
			fileManager.load();
		}
		catch (Exception e)
		{
	...
	private void layoutComponents()
	{
	    ...
	    eastContainer.setBorder(BorderFactory.createEmptyBorder(20,20,20,20));
